...
def deal_logs(intervals, info, idx, name):
    ...
    for chunk_idx, (s, e) in enumerate(intervals):
        if (chunk_idx + 1) % 100 == 0:
            print(
                "Computing Hawkes of chunk {}/{}".format(chunk_idx + 1, len(intervals))
            )
        try:
        ...
        for service, sgroup in service_events:
            ...
            for event, egroup in events:
                ...
                adds = np.array(
                    [idx * (1e-5) for idx in range(len(tmp))]
                )  # In case of too many identical numbers
                knots[eid] = tmp + adds
...
def deal_metrics(intervals, info, idx, name, chunk_length):
    ...
    for nid, service in enumerate(info.service_names):
        df = pd.read_csv(
            os.path.join("./parsed_data", name, "metrics" + idx, service + ".csv")
        )
