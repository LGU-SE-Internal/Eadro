...
def get_basic(info, idx, name, chunk_length=10, threshold=1, **kwargs):
    ...
    print(
        "# starts at {}/{} and ends at {}/{}".format(
            intervals[0][0], start, intervals[-1][-1], end
        )
    )
...
def get_all_chunks(name, chunk_length=10, **kwargs):
    ...
    while True:
        if os.path.exists(
            os.path.join("./parsed_data", name, "records" + str(idx) + ".json")
        ):
...
def split_chunks(name, test_ratio=0.2, concat=False, **kwargs):
    ...
    print(
        "# train chunks: {}/{} ({:.4f}%)".format(
            sum(train_labels), train_num, 100 * (sum(train_labels) / train_num)
        )
    )
    print(
        "# test chunks: {}/{} ({:.4f}%)".format(
            sum(test_labels), test_num, 100 * (sum(test_labels) / test_num)
        )
    )
    print(
        "# total chunk: {}/{} ({:.4f}%)".format(
